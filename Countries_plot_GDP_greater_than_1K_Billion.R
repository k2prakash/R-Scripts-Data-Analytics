...
plot (x, y, axes = FALSE, type = "n", 
      ...
      main = "Plot of Indicators of Countries with Annual GDP > $1k Billion")
...
for(i in 1:nrow(gdp_1kplus)) {
  ...
  radius_gdp <- round((country_gdp_value/1000), digits = 3)
...
for(i in 1:nrow(countries_data_frame)) {
  if (i == 1){
    ...
    # Place the Country's GDP value below the Country's name.
    text(x_value+1, y_value - country_text_offset_value -3, 
    ...
    for (j in 1:i) {
      ...
      print(x_value)
      print(y_value)
      print(countries_data_frame[j,1])
      print(countries_data_frame[j,2])
      #if ((abs(x_value - countries_data_frame[j,1]) < 10) && 
      #    (abs(y_value - countries_data_frame[j,2]) < 10 )) {
      #  vicinity_found <- TRUE
      #  break
      #} 
    ...
    if (isTRUE(vicinity_found)) {
      #random_offset <- (ceiling(runif(1, 1.0, 10.0)) * 3)
      random_offset <- i*5
      if ((y_value - random_offset) < 0) {
        random_offset <- i*2
      }
      arrows(x_value,y_value,x_value,y_value-10 - random_offset,length = 0.10,
             lty = 6)
      text_xvalue <- x_value
      text_yvalue <- y_value - 11 - random_offset
      ...
      # Place the Country's GDP value below the Country's name.
      text(x_value+1, y_value -country_text_offset_value-2, labels = gdp_label_vector, pos= text_pos,
           offset = 1, cex = 0.75, col="blue")
    }
    vicinity_found <- FALSE
  }
...
legend("bottomright", inset=.02, title="GDP Units",
       c("1 radial unit = 1000 Billion USD"), horiz=TRUE, cex=0.8)